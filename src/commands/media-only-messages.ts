...
export class MediaOnlyMessagesCommand extends Subcommand {
	...
	public async enableSubcommand(interaction: Subcommand.ChatInputCommandInteraction<'cached'>) {
		...
		await interaction.reply({
			embeds: [
				createInfoEmbed(
					`The <#${channel.id}> channel is now configured to require attachments when sending messages`,
				),
			],
		});
	}
		...
		registry.registerChatInputCommand((builder) =>
			builder
				...
				.addSubcommand((list) =>
					list
						.setName('list')
						.setDescription('Lists all channels that require attachments when sending messages'),
				),
		);
	}
}
